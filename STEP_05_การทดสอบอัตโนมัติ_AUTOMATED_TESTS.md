# 🧪 STEP 05: การทดสอบอัตโนมัติ (Automated Unit Tests with Pytest)

> ⚠️ **คำสั่งสำคัญของ อ.ดร.ปิยะนุช:** ตารางเทสต์ที่กรอกด้วยมือถือว่าโมฆะ (0 คะแนน) ต้องใช้ชุดทดสอบอัตโนมัติ (Automated Tests) เท่านั้น!

โครงงานนี้ใช้ **Pytest** ทดสอบครอบคลุมทุกฟังก์ชันและเงื่อนไข (6/6 ผ่าน 100%):
1. `test_physical_product_deducts_stock`: ตรวจสอบการตัดสต็อกสินค้ากายภาพเมื่อขาย
2. `test_physical_product_cannot_sell_beyond_stock`: ตรวจสอบ Error เมื่อสินค้ากายภาพไม่พอขาย
3. `test_digital_product_unlimited_sales`: ตรวจสอบสินค้าดิจิทัลขายได้ต่อเนื่องไม่จำกัด
4. `test_digital_product_download_access_locked`: ตรวจสอบการล็อกดาวน์โหลดเมื่อยังไม่ชำระเงิน
5. `test_inventory_service_summary`: ตรวจสอบการคำนวณมูลค่ารวมและจำนวนสินค้าในคลัง
6. `test_mixed_order_processing`: ตรวจสอบคำสั่งซื้อที่มีทั้งสินค้าเล่มและอีบุ๊กพร้อมกัน

---

## ซอร์สโค้ด tests/test_inventory.py
```python
# -*- coding: utf-8 -*-
import pytest
